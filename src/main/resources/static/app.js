...
async function loadNetInfo(){
  let d=await api('/api/networkinfo');if(!d||d.error)return;
  netInfoData=d;
  document.getElementById('ni-pub').textContent=d.publicIp||'N/A';
  document.getElementById('ni-priv').textContent=d.privateIp||'N/A';
  document.getElementById('ni-type').textContent=d.connectionType||'N/A';
  document.getElementById('ni-host').textContent=d.hostname||'N/A';
  // Use geo data from the API response
  let country=d.country&&d.country!=='Unknown'?d.country:'N/A';
  let city=d.city&&d.city!=='Unknown'?d.city:'N/A';
  let region=d.regionName&&d.regionName!=='Unknown'?d.regionName:'N/A';
  // If backend returned Unknown, do a client-side lookup via the public IP
  if(country==='N/A'&&d.publicIp){
    try{
      let geo=await api('/api/lookup/'+d.publicIp);
      if(geo&&!geo.error){
...
// ── Signup Page ──
signup:()=>{generateCaptcha();return `<div class="auth-header" style="padding-bottom:80px">
  <h1><span class="material-symbols-outlined mi text-white" style="font-size:28px">explore</span>GeoTracker</h1>
  <p>Create Account</p>
</div>
<div class="auth-card">
